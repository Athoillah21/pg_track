...
export async function getTableHistory(schema: string, table: string, limit = 50, searchPk?: string): Promise<ChangeHistory[]> {
  let sql = `
    SELECT h.*,
        (
            SELECT ch.new_data 
  ...
  const params: any[] = [schema, table, limit];

  if (searchPk) {
    sql += ` AND h.row_pk::text ILIKE $4`;
    params.push(`%${searchPk.trim()}%`);
  }

  sql += ` ORDER BY h.changed_at DESC LIMIT $3`;

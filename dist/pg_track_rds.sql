-- pg_track_rds.sql
-- Extension: pg_track (Pure SQL Version for RDS)
-- Schema: pgtrack
-- 
-- INSTRUCTIONS FOR RDS:
-- 1. Connect to your RDS database using a client like DBeaver, pgAdmin, or psql.
-- 2. Run this entire script.
-- 3. That's it! The extension is installed.

BEGIN;

CREATE SCHEMA IF NOT EXISTS pgtrack;

-- ============================================================================
-- SCHEMA AND TABLES
-- ============================================================================

-- Schema pgtrack is created by the extension manager if used as extension, but here we do it above.
...
-- Version function (Pure SQL replacement for RDS compatibility)
CREATE OR REPLACE FUNCTION pgtrack.version() RETURNS TEXT AS $$
BEGIN
    RETURN '1.0.0';
END;
$$ LANGUAGE plpgsql IMMUTABLE;
...
-- Done!
SELECT 'pg_track installed successfully on RDS! Version: ' || pgtrack.version() AS result;
